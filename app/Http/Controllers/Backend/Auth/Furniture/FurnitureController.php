...
namespace App\Http\Controllers\Backend\Auth\Furniture;
...
class FurnitureController extends Controller
{
    ...
    {
        return view('backend.auth.furniture.index')
            ->withProducts($this->productRepository->getActiveAuthPaginated(25, 'id', 'asc', 'Muebles'));
    }
    ...
    {
        return view('backend.auth.furniture.create')
    ...
    {   
        ...
        return redirect()->route('admin.auth.donation.furniture.index')->withFlashSuccess(__('alerts.backend.products.created'));
    }
    ...
    {
        return view('backend.auth.furniture.show')
            ->withProduct($product);
    }
    ...
    public function edit(Product $product, ManageProductRequest $request, UserRepository $userRepository, PermissionRepository $permissionRepository)
    {
        ...
        return view('backend.auth.furniture.edit')
            ->withProduct($product)
            ->withUsers($userRepository->with('permissions')->get());
    }
